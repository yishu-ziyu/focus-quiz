...
这不是一份严格的文献综述，而是一份产品设计备忘：为什么 Focus Quiz 不做摘要，而是做三道有阻力的问题；为什么题不能太多；为什么答错后要能回到原文。
...
Focus Quiz 的设计不是为了把阅读变成考试，而是为了在阅读结束后加一个很短的认知阻力。三道题分别检查概念、逻辑和迁移：少于三道不成闭环，多于三道容易变成负担。答错之后还能回到原文，是因为这篇文章本来就是你选择认真读的东西，重读它本身就是学习的一部分。
...
- 三道题可以被解释为一个轻量产品选择：比一道题更可靠，又不把文章阅读变成完整考试。

分享会可以这样说：

> 我没有找到一篇论文严格证明“读完文章后三道题就是最优”。但有研究专门看 review question 的数量，发现超过一道选择题后学习收益明显增加，同时题量收益可能会饱和。我的产品选择是把这个证据和认知负荷理论结合起来：三道题不是数学最优，而是产品上的最小有效剂量。
