# Focus Quiz 发布前体检清单

目标：下周三晚分享前，让项目达到“别人能安装、能理解、能试用、仓库可信”的状态。

## 已发现并处理

- [x] `manifest.json` 缺少云端模型 API 域名权限，可能导致 OpenAI / Gemini / Claude / 国内兼容接口无法从扩展页正常请求。
- [x] 图标文件虽然叫 `.png`，实际是 JPEG 且尺寸全是 1024x1024，已转换为真实 `16/48/128` PNG。
- [x] `src/` 目录是未被当前 manifest 引用的旧版实现，已移除，避免公开仓库结构混乱。
- [x] 侧边栏部分错误提示和模型解释使用 `innerHTML`，已改为 DOM 安全渲染。
- [x] 答题后按钮没有真正禁用，可能重复触发，已补上禁用状态。
- [x] OpenAI-Compatible Provider 若不支持 `response_format`，现在会自动降级重试。
- [x] README 存在错字和动机缺口，已补充认知负荷、检索练习、反事实和迁移应用的产品动机。
- [x] Provider 从固定 8 家扩展为预设注册表，新增 OpenRouter、302.AI、SiliconFlow、Moonshot/Kimi、火山方舟、Groq、Together、Fireworks、MiniMax Token Plan / Coding Plan 和自定义兼容接口。
- [x] MiniMax 从旧 `MiniMax-M1-80k` 更新到 `MiniMax-M2.7` / `MiniMax-M2.7-highspeed` 等 M2 系列模型，并改走 Anthropic-Compatible 接入。
- [x] 个性化认知画像 MVP：记录完整答题事件，按概念边界、因果推演、迁移应用计算本地画像，并在三题结束后给出本轮诊断。
- [x] 自适应 prompt MVP：根据本地画像给下一轮题目注入目标难度和薄弱维度提示。
- [x] 全文模式 MVP：点击插件图标抓取当前网页正文，适合读完整篇文章后接受压力测试。
- [x] 错题回链：错题本中保留原文 URL 和标题，复习错题时可以回到原文重读。
- [x] 三题设计说明：补充认知负荷、主动回忆、必要难度和选择过载的设计依据。
- [x] 自适应题量 MVP：冷启动三题，积累画像后允许模型根据文章类型和薄弱维度生成 1-3 道题。
- [x] 分享稿草稿：补充产品动机、Anki 启发、认知负荷、个性化题量和错题回链的讲法。
- [x] 生成失败恢复：模型返回非法 JSON、空题目或临时失败时，侧边栏提供“重试生成”。
- [x] Hint / Evidence 分离：作答前只显示不剧透的 Hint，作答后再显示证据片段；错题本保存证据片段或来源提示。
- [x] 错题 Markdown 导出：错题本可复制为 Markdown，也可保存为 `.md` 文件，方便进入 Obsidian / Anki 等学习系统。
- [x] 模式入口显性化：侧边栏提供选区模式和全文模式两个可点击入口。
- [x] Hint 开关：用户可以隐藏或重新显示作答前提示，并保存偏好。
- [x] 冷启动阈值调低：完成约 6 道题后开始进入自适应判断。

## 待真人体验验证

- [ ] 第一次安装后，设置页是否足够清楚。
- [ ] 未配置 API Key 时，侧边栏错误提示是否能让用户顺利去设置。
- [ ] 选中文本过短、过长、或者结构混乱时，题目质量是否稳定。
- [ ] 三类题目是否真的有“审问感”，还是仍然像普通阅读理解。
- [ ] 错题本的记录、回看、分享文本是否有继续使用的动力。
- [ ] 错题 Markdown 复制和 `.md` 文件导出格式是否适合直接放入 Obsidian 或改造成 Anki 卡片。
- [ ] Hint 是否足够有帮助但不剧透；Evidence 是否真的对应原文关键论证，还是只是模型随意截取。
- [ ] 用户是否能在不阅读说明文档的情况下理解选区模式和全文模式。
- [ ] 本地 Ollama 的 403 / 模型不存在 / 超时提示是否足够可操作。
- [ ] 自定义 OpenAI-Compatible / Anthropic-Compatible 服务的 Base URL 填写说明是否足够清楚。
- [ ] 宽 HTTPS host permission 是否需要在分享时主动解释。
- [ ] 冷启动时“样本不足”的文案是否自然，不会让第一次使用者困惑。
- [ ] 本轮诊断是否真的符合用户主观感受，是否过度武断。
- [ ] 自适应难度是否能被肉眼感知，还是需要更强的题目差异。
- [ ] 自适应题量是否自然：只出一道题时，用户是否觉得“刚好切中要害”，还是觉得题太少。
- [ ] 本轮策略卡片的文案是否清楚，是否会让用户觉得模型在过度解释。
- [ ] 全文模式对不同网站正文抓取是否稳定，是否会抓到导航、评论或广告。
- [ ] 错题回链是否能稳定打开原文；如果文章需要登录或动态渲染，需要给出提示。

## 分享前建议演示流程

1. 打开一篇文章，选中一段 200-600 字的核心段落。
2. 右键选择 **Focus Quiz: 审问选中文本**。
3. 展示本轮策略卡片：为什么这次是三题完整检查，或为什么只出一道靶向题。
4. 展示题目下方的 Hint，说明作答前不会直接泄露原文证据。
5. 故意选错一道题，展示“思维断裂点”、Evidence 和错题本回链。
6. 在错题本中展示“复制 Markdown”和“保存 .md”，说明它能进入 Obsidian / Anki。
7. 打开设置页，说明 BYOK、多 Provider、本地 Ollama。

## 体验反馈记录格式

你上楼体验后，可以直接按这个格式告诉我：

- 场景：我在哪里点了什么。
- 预期：我以为它应该怎样。
- 实际：它让我不爽的地方是什么。
- 严重度：阻塞 / 明显不爽 / 小修小补。
