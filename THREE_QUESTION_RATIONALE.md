...
更新：三道题现在被定义为**冷启动基线**，不是永远固定的题量。它适合在系统还不了解用户时建立最小诊断闭环；当本地画像积累后，Focus Quiz 会根据文章类型和用户薄弱维度，把本轮题量压缩到 1-3 道。

## 产品判断

Focus Quiz 的目标不是把文章变成一套完整试卷，而是在用户读完文章后，用最小的额外动作判断“我是真的理解了，还是只是看过了”。

如果题目太少，测不出结构性理解；如果题目太多，用户会觉得自己本来只是想读一篇文章，却被迫进入考试状态，最终可能直接放弃。所以三道题是一个冷启动折中：足够形成梯度，又不至于压垮阅读后的行动意愿。

但成熟版本不应该把“三道题”变成教条。如果系统已经知道用户在“场景迁移”上明显薄弱，而当前文章又正好是迁移性很强的概念文章，那么一道高质量迁移题可能比三道平均分配的题更有效。这里的目标不是题量最大化，而是最小有效剂量。
...
1. 系统已经积累了足够答题记录，能判断用户最薄弱的认知维度。
2. 当前文章的主要认知要求高度匹配这个薄弱维度。

例如：用户长期在“场景迁移”题上表现不稳，而一篇文章本身讲的是把某个概念迁移到新制度、新技术或新市场中。这时只出一道迁移题，既降低用户启动成本，也更准确地切中学习收益最高的位置。

这也是个性化的核心意义：不是让用户做更多题，而是让用户做更该做的题。

## 研究依据

- 最接近“三题数量”这个问题的是 Pitt 和 Huebner 的研究。它不是文章阅读场景，也没有证明“三题严格最优”，但它直接研究 review question 的数量与效果：超过一道选择题时，学习表现提升最大，同时也提示题量收益可能存在饱和。这可以支持“冷启动三题”和“自适应最小有效剂量”的产品解释。
...
所以我一开始选择三道题：第一题看概念有没有误读，第二题看因果关系能不能被反事实检验，第三题看能不能迁移到新场景。三道题刚好构成一个冷启动最小闭环：概念、逻辑、迁移。

但如果系统已经知道我最薄弱的是迁移，而这篇文章又特别适合考迁移，那下一轮可以只出一道迁移题。题量变少，学习反而可能更准。这就是我说的个性化：不是把题越做越多，而是找到对当前用户帮助最大的那一道题。
